import time
class SPL06:#固定16hz刷新率 16倍过采样（标准） 模式
    def __init__(self,i2c,addr=0x76):#read:0xef write:0xee(也有可能是0x77,到时候scan一下) 
        ...
        self.i2c.writeto(self.addr,b'\x06\x44')#设置prs_cfg 16hz 16*sample
        self.i2c.writeto(self.addr,b'\x07\xc0')#设置tmp_cfg 16hz 1*sample
    ...
    def prse(self,comp=True):#Praw_sc=Praw/253952 (16倍过采样)
        ...
        p=int.from_bytes(rawreg,'big')#当p为正数时，忽略补码与原码的区别
        if(rawreg[0]&0x80!=0):#若符号位为1，需要从补码转为原码
            p=p-16777216#p-2**24
        p=p/253952#scale
        if (comp):#校准补偿
